//UIUC CS125 FALL 2013 MP. File: MazeRunner.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2013-11-12T09:42:21-0600.264411210
/**
 * 
 * @author Dwoskin2
 *
...
//		int stringLength = safeStringLength(paths[lo]);
//		if (stringLength > findShortestString(paths, lo+1, hi))
//			stringLength = findShortestString(paths, lo+1, hi);
//		return stringLength; // TODO: findShortestString
	}
	...
	static int safeStringLength(String s) {
		if (s != null)
			return s.length();
		return Integer.MAX_VALUE;//TODO: safeStringLength
	}
	...
	public static String shortestPath(int x, int y, int tX, int tY, boolean blocked[][]) {
		if (x < 0 || y < 0 || x > blocked.length -1 || y > blocked[0].length -1){return null;}
		...
		if (paths[shortest] != null)
			return  dir[shortest] + paths[shortest];
